...
- 未开通权益：无法直接调用代理提醒接口。两条出路：① 向华为申请"代理提醒开放能力"（审核约 **8 个工作日**，通过后**调试与发布必须使用手动签名**）；② 改用 Calendar Kit 替代。
...
- 单个普通应用：有效/未过期提醒 **≤ 30 个**。
- 系统总上限：API ≥ 10 时 12000 个；API ≤ 9 时 2000 个。
- "有效"判定：一次性提醒到点后未点 CLOSE 仍占名额；**周期性提醒（如每日重复）无论是否点 CLOSE 永远有效**。KeepFresh 单条每日提醒只占 1 个名额，无压力。
...
- 运行时通知授权：仍需先通过 `notificationManager.requestEnableNotification` 获得通知授权（现有 `NotificationService` 已实现）。
- 模块：`@kit.BackgroundTasksKit` 的 `reminderAgentManager`。
- 模拟器支持：**API 20 起**模拟器才支持代理提醒调试（本项目 API 24，可模拟器验证接口调用，但管控权益需真机验证）。
...
1. 保持现有闹钟型每日提醒实现形态。
2. `publishReminder` 失败时：记录 + 首次失败可弹一次提示（不重复骚扰），确保 `NotificationService` 启动通知兜底。
3. 设置页：提醒开关/时间已由 `ReminderSettings` 支持（preferences 持久化）；开关关闭时 `cancelAllReminders()`（现有实现已做）。
4. 若用户后续希望"内容每天自动更新而不依赖打开 App"：此场景**无解**（更新必须 App 进程执行），接受快照语义或申请权益后配合系统刷新策略评估。
...
- 未查证事项：未开通权益时的**具体错误码数值**、Calendar Kit 替代方案的细节——如实标注，待真机验证
